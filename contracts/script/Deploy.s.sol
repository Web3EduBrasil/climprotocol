...
contract Deploy is Script {
    ...
    function run() external {
        ...
        settlementEngine.grantRole(settlementEngine.AUTOMATION_ROLE(), vm.addr(deployerPrivateKey));
        
        factory.grantRole(factory.EVENT_CREATOR_ROLE(), vm.addr(deployerPrivateKey));
        ...
        console.log("\n=== NEXT STEPS ===");
        console.log("1. Add ClimateOracle as consumer to your Chainlink Functions subscription");
        console.log("2. Fund the subscription with LINK tokens (minimum 2 LINK)");
        ...
        string memory addresses = string(abi.encodePacked(
            ...
            "NEXT_PUBLIC_TOKEN_ADDRESS=", vm.toString(address(climateToken)), "\n",
            "NEXT_PUBLIC_FACTORY_ADDRESS=", vm.toString(address(factory)), "\n",
            "NEXT_PUBLIC_POOL_ADDRESS=", vm.toString(address(liquidityPool)), "\n",
            "NEXT_PUBLIC_SETTLEMENT_ADDRESS=", vm.toString(address(settlementEngine)), "\n",
            "NEXT_PUBLIC_ORACLE_ADDRESS=", vm.toString(address(oracle))
        ));
