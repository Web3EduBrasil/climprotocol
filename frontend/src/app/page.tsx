...
const PrecipitationChart = dynamic(
  () => import('@/components/features/PrecipitationChart').then(m => ({ default: m.PrecipitationChart })),
  {
    ssr: false,
    loading: () => (
      <div className="glass rounded-2xl p-5 flex items-center justify-center h-85">
...
function StoryStep({ number, title, desc, icon, accent = false, children }: {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 items-center">
      ...
      <div className="md:col-span-10 glass rounded-2xl p-6 md:p-8 relative overflow-hidden group hover:border-(--border-strong) transition-all duration-500">
        <div className="absolute inset-0 bg-linear-to-r from-(--accent-glow) to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
        <div className="relative z-10 flex flex-col md:flex-row items-start gap-5">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${accent ? 'bg-[#2A5ADA]/15 text-[#2A5ADA]' : 'bg-(--accent-glow) text-(--accent)'}`}>
            {icon}
          </div>
          <div className="flex-1">
            <h3 className="text-lg md:text-xl font-bold text-(--text-primary) mb-2">{title}</h3>
...
function ScrollToTop() {
  ...
  return (
    <button
      ...
      className={`fixed bottom-6 right-6 z-50 w-11 h-11 rounded-full bg-(--accent) text-(--bg-primary) shadow-lg shadow-(--accent)/25 flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-xl cursor-pointer ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'
        }`}
    >
      <HiOutlineChevronUp className="w-5 h-5 stroke-2" />
    </button>
  );
}
  ...
  return (
    <div className="space-y-0">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center -mx-4 sm:-mx-6 lg:-mx-10 xl:-mx-16 -mt-6 px-4 sm:px-6 lg:px-10 xl:px-16 overflow-hidden">
        ...
        {/* Gradient overlays */}
        <div className="absolute inset-0 bg-linear-to-b from-transparent via-transparent to-(--bg-primary) pointer-events-none z-1" />
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-200 h-150 bg-linear-to-b from-(--accent)/8 via-[#2A5ADA]/5 to-transparent rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 text-center max-w-4xl mx-auto py-20">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 bg-(--surface)/60 backdrop-blur-sm border border-(--border) rounded-full px-4 py-1.5 mb-8 animate-fade-in">
            <SiChainlink className="w-3.5 h-3.5 text-[#2A5ADA]" />
            <span className="text-[11px] font-medium text-(--text-muted)">Built for</span>
          ...
          <div className="mx-auto w-72 h-72 md:w-96 md:h-96 lg:w-md lg:h-112 mb-8 animate-float">
            <Image
              ...
              style={{ filter: 'drop-shadow(0 0 30px rgba(255,255,255,0.9)) drop-shadow(0 0 80px rgba(255,255,255,0.6)) drop-shadow(0 0 180px rgba(255,255,255,0.4))' }}
              priority
            />
          </div>

          {/* Main title */}
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-(--text-primary) leading-[1.05] tracking-tight animate-fade-in">
            {t.hero.title}
            <br />
            <span className="gradient-text">{t.hero.titleHighlight}</span>
          </h1>

          <p className="text-base md:text-lg text-(--text-muted) mt-6 max-w-2xl mx-auto leading-relaxed animate-fade-in animate-delay-200">
            {t.hero.subtitle}
          </p>
          ...
          <div className="mt-16 animate-pulse">
            <div className="w-5 h-8 border-2 border-(--text-faint) rounded-full mx-auto flex justify-center pt-1.5">
      ...
      <section className="py-16">
        <RevealSection>
          <div className="text-center mb-10">
            <span className="text-[10px] font-bold text-(--accent) uppercase tracking-[0.2em]">Live On-Chain Data</span>
            <h2 className="text-3xl md:text-4xl font-bold text-(--text-primary) mt-2">Protocol Statistics</h2>
            <p className="text-sm text-(--text-muted) mt-2">Real-time data from Sepolia smart contracts + Chainlink Data Feeds</p>
          </div>
        </RevealSection>
      ...
      <section className="py-16">
        <RevealSection>
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-(--text-primary) mt-2">{t.protection.howItWorks}</h2>
        ...
        <div className="space-y-6 md:space-y-8">
          <RevealSection direction="left">
            ...
            >
              <div className="flex flex-wrap gap-2 mt-3">
                <span className="text-[9px] bg-(--accent-glow) text-(--accent) px-2 py-0.5 rounded-full font-medium">ERC-1155</span>
                <span className="text-[9px] bg-(--surface-input) text-(--text-muted) px-2 py-0.5 rounded-full">Climate Event Factory</span>
              </div>
            </StoryStep>
          </RevealSection>
          ...
          <RevealSection direction="right">
            ...
            >
              <div className="flex flex-wrap gap-2 mt-3">
                <span className="text-[9px] bg-(--accent-glow) text-(--accent) px-2 py-0.5 rounded-full font-medium">Liquidity Pool</span>
                <span className="text-[9px] bg-(--surface-input) text-(--text-muted) px-2 py-0.5 rounded-full">Premium → Collateral</span>
              </div>
            </StoryStep>
          </RevealSection>
          ...
          <RevealSection direction="right">
            ...
            >
              <div className="flex flex-wrap gap-2 mt-3">
                ...
                <span className="text-[9px] bg-(--surface-input) text-(--text-muted) px-2 py-0.5 rounded-full">Settlement Engine</span>
              </div>
            </StoryStep>
          </RevealSection>
        </div>
      ...
      <section className="py-16">
        <RevealSection>
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-(--text-primary) mt-2">CRE Workflow Architecture</h2>
            <p className="text-sm text-(--text-muted) mt-2 max-w-xl mx-auto">
      ...
      <section className="py-16">
        <RevealSection>
          <div className="text-center mb-10">
            <span className="text-[10px] font-bold text-(--accent) uppercase tracking-[0.2em]">Real-Time Oracle Data</span>
            <h2 className="text-3xl md:text-4xl font-bold text-(--text-primary) mt-2">{t.chart.title}</h2>
            <p className="text-sm text-(--text-muted) mt-2">Open-Meteo API → Chainlink Functions → On-Chain Verification</p>
          </div>
        </RevealSection>
      ...
      <section className="py-16">
        <RevealSection>
          <div className="flex items-center justify-between mb-8">
            <div>
              <span className="text-[10px] font-bold text-(--accent) uppercase tracking-[0.2em]">Marketplace</span>
              <h2 className="text-3xl md:text-4xl font-bold text-(--text-primary) mt-1">{t.events.activeEventsTitle}</h2>
            </div>
        ...
        <RevealSection direction="scale">
          {activeEvents.length === 0 ? (
            <div className="glass rounded-2xl p-14 text-center relative overflow-hidden">
              <div className="absolute inset-0 bg-linear-to-br from-(--accent-glow) to-transparent pointer-events-none" />
              ...
              {activeEvents.slice(0, 6).map((event) => (
                <div key={event.id} className="stagger-child glass rounded-2xl p-5 card-hover">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <p className="text-[10px] text-(--text-faint) uppercase">{t.events.threshold}</p>
                      <p className="text-xs font-medium text-(--text-secondary)">&lt; {event.thresholdMm} mm</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-(--text-faint) uppercase">{t.events.tokens}</p>
                      <p className="text-xs font-medium text-(--text-secondary)">{event.availableTokens}/{event.totalSupply}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-(--text-faint) uppercase">{t.protection.payout}</p>
                      <p className="text-xs font-medium text-(--accent)">{Number(formatEther(event.payoutPerToken)).toFixed(3)} ETH</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <section className="pb-16">
          ...
          <RevealSection>
            <div className="glass rounded-2xl p-6">
              <h3 className="text-sm font-semibold text-(--text-primary) mb-4">{t.portfolio.title}</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {portfolio.map(item => (
                      <tr key={String(item.eventId)} className="border-b border-(--border) last:border-0">
                        <td className="py-3 pr-4 text-(--text-primary) font-medium">{item.eventName}</td>
                        <td className="py-3 pr-4 text-(--text-secondary)">{item.tokenBalance}</td>
                        <td className="py-3 pr-4 text-(--accent)">{Number(formatEther(item.potentialPayout)).toFixed(4)} ETH</td>
                        <td className="py-3">
