    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-sm text-(--text-muted)">Loading settled events...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <HiOutlineScale className="w-6 h-6 text-(--accent)" />
          <h1 className="text-2xl font-bold text-(--text-primary)">{t.settlement.title}</h1>
        </div>
        <p className="text-sm text-(--text-muted)">{t.settlement.subtitle}</p>
      </div>
      ...
      <div className="glass rounded-2xl p-5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xs font-bold text-(--text-primary) uppercase tracking-wider">CRE Settlement Workflow</h3>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2">
            {[
              ...
              <div key={step.label} className="flex items-center gap-2 shrink-0">
                <div className="bg-(--surface-input) rounded-xl px-3 py-2 border border-(--border) min-w-[110px] text-center hover:border-(--accent) transition-colors">
                  <p className="text-[10px] font-bold" style={{ color: step.color }}>{step.label}</p>
                  <p className="text-[9px] text-(--text-muted) mt-0.5">{step.sub}</p>
                </div>
                {i < arr.length - 1 && (
                  <svg className="w-4 h-4 text-(--text-faint) shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                )}
              </div>
            ))}
          </div>
        ...
        {settledEvents.map(event => {
          ...
          return (
            <div key={event.id} className="glass rounded-2xl overflow-hidden card-hover">
              <div className={`px-5 py-3 flex items-center gap-2 ${event.payoutTriggered ? 'bg-linear-to-r from-green-500/10 to-green-500/5 border-b border-green-500/10' : 'bg-linear-to-r from-gray-500/10 to-gray-500/5 border-b border-gray-500/10'}`}>
              ...
              <div className="p-5">
                ...
                <div className="bg-(--surface-input) rounded-xl p-5 mb-5">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-(--text-muted)">{t.settlement.recordedVsThreshold}</span>
                    <span className="text-xs text-(--text-muted)">{t.settlement.dataVia} <span className="text-(--chainlink) font-medium">Chainlink Functions</span></span>
                  </div>
                  <div className="relative h-12 bg-(--bg-primary) rounded-lg overflow-hidden mb-3">
                    ...
                    <div className={`absolute top-0 bottom-0 left-0 rounded-r-lg transition-all duration-1000 ${event.payoutTriggered ? 'bg-linear-to-r from-green-500/30 to-green-500/50' : 'bg-linear-to-r from-gray-500/30 to-gray-500/50'}`} style={{ width: `${Math.min(((event.actualMm || 0) / 300) * 100, 100)}%` }}>
                      <div className="absolute right-2 top-1/2 -translate-y-1/2 text-sm font-bold text-(--text-primary)">{event.actualMm} mm</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.recorded}</p>
                      <p className={`text-lg font-bold ${event.payoutTriggered ? 'text-green-400' : 'text-gray-400'}`}>{event.actualMm} mm</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.threshold}</p>
                      <p className="text-lg font-bold text-red-400">{event.thresholdMm} mm</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.difference}</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                  <div className="bg-(--surface-input) rounded-xl p-3 text-center">
                    <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.supply}</p>
                    <p className="text-sm font-bold text-(--text-primary)">{event.totalSupply} CET</p>
                  </div>
                  <div className="bg-(--surface-input) rounded-xl p-3 text-center">
                    <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.payoutPerToken}</p>
                    <p className="text-sm font-bold text-(--text-primary)">{Number(formatEther(event.payoutPerToken)).toFixed(3)} ETH</p>
                  </div>
                  <div className="bg-(--surface-input) rounded-xl p-3 text-center">
                    <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.totalPayout}</p>
                    <p className={`text-sm font-bold ${event.payoutTriggered ? 'text-(--accent)' : 'text-gray-400'}`}>
                      {event.payoutTriggered ? `${(Number(formatEther(event.payoutPerToken)) * event.totalSupply).toFixed(2)} ETH` : '0 ETH'}
                    </p>
                  </div>
                  <div className="bg-(--surface-input) rounded-xl p-3 text-center">
                    <p className="text-[10px] text-(--text-faint) uppercase">{t.settlement.resultLabel}</p>
                ...
                {isConnected && hasTokens && event.payoutTriggered && (
                  <div className="bg-linear-to-r from-(--accent-glow) to-green-500/5 rounded-xl p-4 border border-(--border)">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold text-(--text-primary)">{t.settlement.youHave} {userItem!.tokenBalance} {t.settlement.tokensToRedeem}</p>
                        <p className="text-xs text-(--accent) mt-0.5">{t.protection.payout}: {Number(formatEther(userItem!.potentialPayout)).toFixed(4)} ETH</p>
                      </div>
      ...
      {settledEvents.length === 0 && (
        <div className="glass rounded-2xl p-12 text-center">
          <HiOutlineScale className="w-12 h-12 text-(--text-faint) mx-auto mb-3" />
          <p className="text-sm text-(--text-muted)">{t.settlement.noSettled}</p>
          <p className="text-xs text-(--text-faint) mt-1">{t.settlement.autoSettlement}</p>
        </div>
      )}
    </div>
  );
