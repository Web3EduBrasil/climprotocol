...
var configSchema = exports_external.object({
  ...
  openMeteoBaseUrl: exports_external.string(),
  evms: exports_external.array(exports_external.object({
    protocolAddress: exports_external.string(),
    settlementAddress: exports_external.string(),
...
var getActiveEvents = (runtime2, evmConfig) => {
  ...
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: evmConfig.settlementAddress,
      data: callData
    }),
...
var checkUpkeep = (runtime2, evmConfig) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: evmConfig.chainSelectorName,
    isTestnet: true
...
var fetchPrecipitation = (sendRequester, config, latitude, longitude, startDate, endDate) => {
  const url = `${config.openMeteoBaseUrl}?latitude=${latitude}&longitude=${longitude}` + `&start_date=${startDate}&end_date=${endDate}` + `&daily=precipitation_sum&timezone=UTC`;
  ...
  const meteoData = JSON.parse(responseText);
  if (!meteoData.daily || !meteoData.daily.precipitation_sum || meteoData.daily.precipitation_sum.length === 0) {
    throw new Error("Open-Meteo returned no precipitation data");
  }
  const totalPrecipitationMm = meteoData.daily.precipitation_sum.reduce((sum, val) => sum + (val || 0), 0);
  return {
    totalPrecipitationMm,
    days: meteoData.daily.precipitation_sum.length
  };
};
var triggerSettlement = (runtime2, evmConfig, performData) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: evmConfig.chainSelectorName,
    isTestnet: true
  ...
  runtime2.log(`Settlement tx succeeded: ${bytesToHex(txHash)}`);
...
var timestampToDate = (timestamp) => {
  ...
  const daysInYear = (yr) => isLeap(yr) ? 366 : 365;
...
var onCronTrigger = (runtime2, payload) => {
  ...
  for (const eventId of activeEventIds) {
    try {
      const { eventData, isSettled } = getEventDetails(runtime2, evmConfig, eventId);
      ...
      runtime2.log(`Event ${eventId}: lat=${lat}, lon=${lon}, ` + `end=${timestampToDate(eventData.endTime)}, ` + `threshold=${thresholdMm}mm, ` + `status=${eventData.status}, settled=${isSettled}`);
      if (eventData.status === EventStatus.ACTIVE && !isSettled && currentTimestamp > eventData.endTime) {
        expiredEvents.push({ eventId, eventData });
      }
    } catch (error) {
      runtime2.log(`Error reading event ${eventId}: ${String(error)}`);
    }
  }
  runtime2.log(`Events ready for settlement: ${expiredEvents.length}`);
  if (expiredEvents.length === 0) {
    runtime2.log("No expired events need settlement.");
  ...
  for (const { eventId, eventData } of expiredEvents) {
    ...
    const thresholdMm = Number(eventData.thresholdMm) / 1000;
    try {
      const precipResult = httpClient.sendRequest(runtime2, (sendReq, config) => fetchPrecipitation(sendReq, config, lat, lon, startDate, endDate), ConsensusAggregationByFields({
        totalPrecipitationMm: median,
        days: median
      }))(runtime2.config).result();
      const willPayout = precipResult.totalPrecipitationMm < thresholdMm;
      runtime2.log(`Event ${eventId} precipitation data:` + `
  ...
  Payout triggered: ${willPayout ? "YES — drought detected!" : "NO — sufficient rain"}`);
      ...
      runtime2.log(`Failed to fetch precipitation for event ${eventId}: ${String(error)}`);
    }
  }
  try {
    const { upkeepNeeded, performData } = checkUpkeep(runtime2, evmConfig);
    if (upkeepNeeded) {
      runtime2.log("checkUpkeep returned TRUE — triggering settlement...");
      const txHash = triggerSettlement(runtime2, evmConfig, performData);
      runtime2.log(`Settlement triggered successfully. TxHash: ${txHash}`);
      return `settlement_triggered:${txHash}`;
    } else {
      runtime2.log("checkUpkeep returned FALSE — settlement already in progress or oracle pending.");
    }
  } catch (error) {
    runtime2.log(`Settlement trigger error: ${String(error)}`);
  }
  return `analyzed:${expiredEvents.length}_events`;
};
var onSettlementCompleted = (runtime2, payload) => {
  runtime2.log("=== SettlementCompleted Event Detected ===");
  const topics = payload.topics;
  if (topics.length < 2) {
    runtime2.log("Unexpected log format: not enough topics");
  ...
  runtime2.log(`Settlement completed for event ${eventId}`);
  const evmConfig = runtime2.config.evms[0];
  try {
    const { eventData, isSettled } = getEventDetails(runtime2, evmConfig, eventId);
    ...
    const lat = Number(eventData.latitude) / 1e6;
    const lon = Number(eventData.longitude) / 1e6;
    runtime2.log(`Settlement outcome:` + `
  Event ID: ${eventId}` + `
  Location: (${lat}, ${lon})` + `
  ...
  Status: ${isSettled ? "SETTLED" : "PENDING"}`);
    ...
    runtime2.log(`Error reading event details: ${String(error)}`);
  }
...
var initWorkflow = (config) => {
  ...
  if (!network248) {
    throw new Error(`Network not found for chain: ${config.evms[0].chainSelectorName}`);
  }
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  return [
